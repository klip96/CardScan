...
# Актуально с тех пор, как вход доступен не только из локальной сети, но и
# через интернет (ngrok/публичный домен). Простой лимит попыток по IP —
# в памяти процесса, без БД: сброс при перезапуске сервера не критичен.
...
def client_ip(request: Request) -> str:
    """IP клиента с учётом ngrok/обратного прокси (X-Forwarded-For)."""
...
def check_rate_limit(key: str) -> bool:
    """True, если попытка входа разрешена (лимит не превышен)."""
    now = time.time()
    attempts = [t for t in _failed_attempts.get(key, []) if now - t < _RATE_LIMIT_WINDOW_SEC]
